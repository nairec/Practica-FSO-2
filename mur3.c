...
char id_mem_s[20], id_sem_curses_s[20], id_sem_memoria_s[20], id_mis_s[20],n_fil_s[20], n_col_s[20], n_pal_s[20], m_por_s[20], c_pal_s[20], m_pal_s[20], pos_f_s[20], pos_c_s[20], vel_f_s[20], vel_c_s[20], ball_id_s[20], retard_s[20], nblocs_offset_s[20], npilotes_offset_s[20], final_joc_offset_s[20];
...
int carrega_configuracio(FILE * fit)
{
	...
	n_paletes = i;
	 ...
	 /* Si no hi ha línies P al fitxer, deixem n_paletes == 0 i crearem
		 la paleta per defecte més endavant a inicialitza_joc(), després
		 d'haver inicialitzat les dimensions automàtiques (params_auto). */
	...
	for (int j = 0; j < n_paletes; j++) {
		if ((paletes[j].amplada < 1) || (paletes[j].amplada >= n_col - 3) || (paletes[j].col_inicial < 1) 
		|| (paletes[j].col_inicial + paletes[j].amplada > n_col - 1) || (paletes[j].fila < 1) || (paletes[j].fila >= n_fil - 1)) {
			ret = 5;
			break;
		}
	}
...
int inicialitza_joc(void)
{
	...
	final_joc_offset = mida_mem + 2*sizeof(int); // bandera de fi al final
	...
	*p_final_joc = 0;
	...
	for (int p = 0; p < n_paletes; p++) {
		...
		for (int i = 0; i < paletes[p].amplada; i++) {
			win_escricar(paletes[p].fila, paletes[p].col_inicial + i, simbol, INVERS);
		}
	}
	...
	f_pil = pos_f;
	c_pil = pos_c;
	win_escricar(f_pil, c_pil, '1', NO_INV);
...
int main(int n_args, char *ll_args[])
{
	...
	for (int i = 0; i < n_paletes; i++) {
		pthread_create(&paletes[i].thread_id, NULL, mou_paleta_thread, &paletes[i]);
	}
	...
	do
	{
		...
		if (tecla == TEC_RETURN) {
			...
			for (int i = 0; i < n_paletes; i++) {
				if (paletes[i].id == id_paleta) {
					paletes[i].salt_vertical = 1;
					break;
				}
			}
		}
	...
	*p_final_joc = 1;
	...
	final_joc = 1;
	for (int i = 0; i < n_paletes; i++) {
		pthread_join(paletes[i].thread_id, NULL);
	}
